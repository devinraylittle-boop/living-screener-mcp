...
def _status_class(status: str | None) -> str:
    ...
    if value in {"BROKER_PROOF_MANUAL_READY", "BROKER_PROOF_AUTONOMY_READY", "BROKER_PROOF_READY", "BROKER_AUTONOMY_PROOF_READY", "PROVEN", "PROVEN_READY", "REAL_CASH_PROOF_READY", "REVIEW_ONLY_OPTIONS_READY", "SMALL_ACCOUNT_SCALP_ACCEPTABLE", "OPTIONS_CHAIN_ACCEPTABLE", "EVENT_VOLATILITY_PLAYBOOK_READY", "EVENT_OPTIONS_READY", "EVENT_RADAR_READY", "BROAD_OPTIONS_READY", "DATA_TRUTH_READY", "SYSTEM_COMMUNICATION_AUDIT_READY"}:
        return "ok"
    if value in {"BROKER_PROOF_INCOMPLETE", "BROKER_PROOF_BLOCKED", "BROKER_AUTONOMY_BLOCKED", "MISSING", "BLOCKED", "REAL_CASH_BLOCKED", "AUTONOMOUS_EXECUTION_BLOCKED", "BLOCKED_BY_DATA", "NO_TRADE_PLAN", "NO_SMALL_ACCOUNT_CONTRACT", "EVENT_NO_TRADE_PLAN", "BROAD_NO_TRADE_PLAN", "DATA_TRUTH_BLOCKED"}:
        return "bad"
    return "warn"
...
def _crypto_live_test_html(payload: dict[str, Any]) -> HTMLResponse:
    result = payload.get("result") or {}
    gates = result.get("exchange_gates") or {}
    rows = []
    for item in result.get("candidate_classifications") or []:
        ticket = item.get("order_ticket") or {}
        rows.append(
            "<tr>"
            f"<td>{escape(str(item.get('symbol') or ''))}</td>"
            f"<td>{escape(str(item.get('classification') or ''))}</td>"
            f"<td>{escape(str(item.get('setup_score') or 0))}</td>"
            f"<td>{escape(str(item.get('liquidity_score') or 0))}</td>"
            f"<td>{escape(str(item.get('spread_score') or 0))}</td>"
            f"<td>{escape(str(ticket.get('intended_limit_price') or ''))}</td>"
            f"<td>{escape(str(ticket.get('max_loss') or ''))}</td>"
            f"<td>{escape('; '.join(item.get('approval_or_rejection_reasons') or []))}</td>"
            "</tr>"
        )
    body = f"""
<div class="topbar">
  <div>
    <h1>Crypto Live Test Gate</h1>
    <p>Autonomous crypto readiness gate. This MCP still cannot place or cancel exchange orders.</p>
  </div>
  <div><span class="badge warn">{escape(str(result.get('final_decision') or 'REVIEW_ONLY'))}</span></div>
</div>
{_field_grid([
    ("Build", payload.get("build_version")),
    ("Status", result.get("status")),
    ("Gate Status", gates.get("status")),
    ("Approved Candidates", result.get("approved_candidate_count")),
    ("Can Place Orders", payload.get("can_place_order_from_this_mcp")),
])}
<h2>Blockers</h2>
{_list(gates.get("blockers") or [])}
<h2>Candidates</h2>
<table>
  <thead>
    <tr><th>Symbol</th><th>Class</th><th>Setup</th><th>Liquidity</th><th>Spread</th><th>Limit</th><th>Max Loss</th><th>Reason</th></tr>
  </thead>
  <tbody>{''.join(rows) if rows else '<tr><td colspan="8">No candidates returned.</td></tr>'}</tbody>
</table>
<h2>Next Action</h2>
<p>{escape(str(result.get('next_action') or 'PASS'))}</p>
"""
    return _html_page("Crypto Live Test Gate", body, payload)


def _crypto_test_report_html(payload: dict[str, Any]) -> HTMLResponse:
    result = payload.get("result") or {}
    body = f"""
<div class="topbar">
  <div>
    <h1>Crypto Test Report</h1>
    <p>Post-test summary for live or no-trade crypto validation.</p>
  </div>
  <div><span class="badge ok">{escape(str(result.get('final_decision_for_tomorrow') or 'REVIEW_ONLY'))}</span></div>
</div>
{_field_grid([
    ("Build", payload.get("build_version")),
    ("Starting Balance", result.get("starting_balance")),
    ("Ending Balance", result.get("ending_balance")),
    ("P/L", result.get("total_profit_loss")),
    ("Trades", result.get("trade_count")),
    ("Rejected", result.get("rejected_candidate_count")),
    ("Fees", result.get("fees_paid")),
    ("Can Place Orders", payload.get("can_place_order_from_this_mcp")),
])}
<h2>Module Decisions</h2>
{_field_grid(list((result.get("module_decisions") or {}).items()))}
<h2>Tomorrow Improvements</h2>
{_list(result.get("tomorrow_stock_options_improvements") or [])}
"""
...
def _broker_proof_bridge_html(payload: dict[str, Any]) -> HTMLResponse:
    result = payload.get("result") or {}
    rows = []
    for item in result.get("proof_items") or []:
        rows.append(
    ...
    body = f"""
      <div class="topbar">
        <div>
          <h1>Broker Proof Bridge</h1>
          <p>Turns broker/account/order-preview facts into strict proof items before they are allowed into the real-cash proof gate.</p>
        </div>
        <span class="badge {escape(_status_class(str(result.get("status"))))}">{escape(str(result.get("status")))}</span>
      </div>
      {_field_grid([
          ("Manual Review", decisions.get("manual_real_cash_review")),
          ("Autonomous Execution", decisions.get("fully_autonomous_real_cash_execution")),
          ("Machine Verified", result.get("machine_verified")),
          ("Ticker", result.get("ticker")),
          ("Contract", result.get("contract_symbol")),
          ("Proof Gate", result.get("proof_gate_link_html")),
      ])}
      <h2>Broker Proof Items</h2>
      <table><thead><tr><th>Category</th><th>Proof</th><th>Status</th><th>Evidence</th></tr></thead><tbody>{"".join(rows)}</tbody></table>
      <h2>Next Step</h2>
      <p>{escape(str(result.get("next_step") or ""))}</p>
    """
    return _html_page("Broker Proof Bridge", body, payload)
...
async def fallback_broker_proof_bridge(request: Request) -> JSONResponse | HTMLResponse:
    params = request.query_params
    result = _get_broker_proof_bridge(
        container,
        _float_or_none(params.get("account_value")) or 100.0,
        _float_or_none(params.get("intended_cash")) or 100.0,
        params.get("ticker") or "",
        params.get("contract_symbol") or "",
        params.get("account_last4") or "",
        params.get("account_type") or "",
        _truthy(params.get("broker_account_confirmed")),
        _truthy(params.get("buying_power_confirmed")),
        _float_or_none(params.get("buying_power")),
        _truthy(params.get("open_orders_checked")),
        _int_or_default(params.get("open_order_count"), -1) if params.get("open_order_count") is not None else None,
        _truthy(params.get("open_positions_checked")),
        _int_or_default(params.get("open_position_count"), -1) if params.get("open_position_count") is not None else None,
        _truthy(params.get("duplicate_order_active")),
        _truthy(params.get("order_preview_confirmed")),
        params.get("preview_order_type") or "limit",
        params.get("preview_side") or "buy",
        _int_or_default(params.get("preview_quantity"), 1),
        _float_or_none(params.get("preview_limit_price")),
        _float_or_none(params.get("preview_max_loss")),
        _truthy(params.get("options_snapshot_validated")),
        _float_or_none(params.get("options_snapshot_age_seconds")),
        params.get("broker_source") or "operator_supplied",
        _truthy(params.get("separate_broker_executor_proven")),
    )
    payload = _review_only_envelope({"result": result})
    if _wants_html(request):
...
async def fallback_crypto_live_test_gate(request: Request) -> JSONResponse | HTMLResponse:
    params = request.query_params
    ...
    if raw_snapshots:
        try:
            ...
            pass
    result = container.crypto_paper.live_test_gate(
        symbols=_tickers(params.get("symbols")),
        starting_cash=_float_or_none(params.get("starting_cash")) or 5.0,
    ...
    payload = _review_only_envelope({"result": result})
    if _wants_html(request):
        return _crypto_live_test_html(payload)
    return JSONResponse(payload)


async def fallback_crypto_test_report(request: Request) -> JSONResponse | HTMLResponse:
    params = request.query_params
    result = container.crypto_paper.summarize_live_test_report(
        starting_balance=_float_or_none(params.get("starting_balance")) or 5.0,
        ending_balance=_float_or_none(params.get("ending_balance")),
        fees_paid=_float_or_none(params.get("fees_paid")) or 0.0,
        estimated_slippage=_float_or_none(params.get("estimated_slippage")) or 0.0,
        limit_events=_int_or_default(params.get("limit_events"), 100),
    )
    payload = _review_only_envelope({"result": result})
    if _wants_html(request):
