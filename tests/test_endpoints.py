...
class EndpointTests(unittest.TestCase):
    def test_health_config_and_version_are_safe(self) -> None:
        ...
        self.assertIn("options_data_provider", config.json())
        self.assertIn("has_marketdata_api_key", config.json())
        self.assertIn("has_tradier_access_token", config.json())
        ...
        self.assertIn("get_strategy_module_registry", tools.json()["tools"])
        ...
        self.assertIn("get_options_data_status", tools.json()["tools"])
        self.assertIn("get_truth_source_status", tools.json()["tools"])
        self.assertIn("check_market_data_health", tools.json()["tools"])
        self.assertIn("get_catalyst_context", tools.json()["tools"])
    ...
    def test_options_data_status_endpoint_is_review_only(self) -> None:
        client = TestClient(create_app())
        response = client.get("/options/data-status")
        ...
        self.assertFalse(response.json()["can_place_order_from_this_mcp"])

    def test_market_truth_endpoints_are_review_only(self) -> None:
        client = TestClient(create_app())
        fake_catalyst = {
        ...
        truth = client.get("/truth/source-status")
        health = client.get("/market/data-health?tickers=SOFI,SMCI&max_tickers=2")
        missing_catalyst = client.get("/research/catalyst-context")
        with patch("app.fallback_endpoints.container.market_truth.get_catalyst_context", return_value=fake_catalyst):
            catalyst = client.get("/research/catalyst-context?ticker=SOFI")

        self.assertEqual(truth.status_code, 200)
        self.assertEqual(truth.json()["result"]["schema_version"], "truth_source_status_v1")
        self.assertFalse(truth.json()["result"]["cash_readiness"]["cash_ready"])
        self.assertFalse(truth.json()["can_place_order_from_this_mcp"])
        self.assertEqual(health.status_code, 200)
        self.assertEqual(health.json()["result"]["schema_version"], "market_data_health_v1")
        self.assertEqual(health.json()["result"]["checked_tickers"], ["SOFI", "SMCI"])
        self.assertFalse(health.json()["can_cancel_order_from_this_mcp"])
        self.assertEqual(missing_catalyst.status_code, 400)
        self.assertFalse(missing_catalyst.json()["can_place_order_from_this_mcp"])
        self.assertEqual(catalyst.status_code, 200)
    ...
    def test_trading_day_alerts_endpoint_renders_attention_queue(self) -> None:
            ...
            "alerts": [
                {
                    ...
                    "ticker": "ORCL",
                    "direction": "short",
                    "contract_symbol": "ORCL260612P00155000",
                    "age_bucket": "current_0_5m",
                    "is_current": True,
        ...
        self.assertIn("ORCL", html.text)
        self.assertIn("Current?", html.text)
        self.assertEqual(json_response.status_code, 200)
    ...
    def test_trading_monster_dashboard_control_and_war_room(self) -> None:
        ...
        self.assertIn("Stock Broker Route", dashboard.text)
        self.assertIn("Living Screener Direct Submit", dashboard.text)
        self.assertIn("YES, GATED", dashboard.text)
        ...
        self.assertIn("get_option_instruments", options_router.json()["result"]["required_options_tools"])
        self.assertIn("get_option_positions", options_router.json()["result"]["required_options_tools"])
        self.assertIn("get_option_orders", options_router.json()["result"]["required_options_tools"])
        self.assertEqual(
            options_router.json()["result"]["available_in_current_codex_session"]["robinhood_trading_mcp_url"],
            "https://agent.robinhood.com/mcp/trading",
        )
        self.assertIn("place_option_order", options_router.json()["result"]["available_in_current_codex_session"]["official_options_tools_expected_after_connection"])
    ...
    def test_debug_validation_endpoints_are_static_and_safe(self) -> None:
        ...
        self.assertEqual(full_html.status_code, 200)
        self.assertIn("Full Health", full_html.text)
        self.assertIn("Options Truth", full_html.text)
        ...
        self.assertTrue(manifest.json()["result"]["required_tools"]["get_strategy_module_registry"])
        ...
        self.assertTrue(manifest.json()["result"]["required_tools"]["get_options_data_status"])
        self.assertTrue(manifest.json()["result"]["required_tools"]["get_truth_source_status"])
        self.assertTrue(manifest.json()["result"]["required_tools"]["check_market_data_health"])
        self.assertTrue(manifest.json()["result"]["required_tools"]["get_catalyst_context"])
        ...
        self.assertEqual(schema.json()["result"]["options_data_status_schema_preview"]["schema_version"], "options_data_status_v1")
    ...
    def test_event_volatility_endpoints_are_readable_and_review_only(self) -> None:
            ...
            "stock_review_candidates": [
                {
                    ...
                    "relative_volume": 1.8,
                    "event_related": False,
                    "why_not_ranked": ["No rankable options review is available in this pass."],
                }
            ],
            "microcap_paper_research": [],
            "crypto_research_lane": {"status": "SEPARATE_PAPER_RESEARCH", "symbols": ["BTC-USD"], "reason": "Separate lane."},
        ...
        self.assertFalse(radar_json.json()["can_place_order_from_this_mcp"])
        self.assertEqual(radar_html.status_code, 200)
        self.assertIn("Event Radar", radar_html.text)
        self.assertIn("Anti-Tunnel-Vision", radar_html.text)
        ...
        self.assertEqual(broad_json.status_code, 200)
        self.assertEqual(broad_json.json()["result"]["status"], "BROAD_STOCK_REVIEW_ONLY")
        self.assertFalse(broad_json.json()["can_place_order_from_this_mcp"])
    ...
    def test_autonomous_firewall_endpoints_are_review_only(self) -> None:
        fake_registry = {
            ...
            "modules": [
                {
                    "module": "clean_liquid_options_scalp",
                    "status": "REVIEW_ONLY",
                    "paper_enabled": True,
                    "cash_autonomous_enabled": False,
                    "reason": "Broker truth required before cash.",
                }
            ],
            "live_performance_memory": {"sample_size": 0},
            "review_only": True,
            "can_place_order_from_this_mcp": False,
            "can_cancel_order_from_this_mcp": False,
        }
            ...
            "supporting_confirmations": [
                {
                    ...
                    "signal_type": "paper_trial",
                    "directional_bias": "short",
                    "intelligence_score": 66,
                    "final_use": "SUPPORTING_CONFIRMATION",
                }
            ],
            "noise_filter_rules": ["Unknown information never becomes confidence."],
            "review_only": True,
            "can_place_order_from_this_mcp": False,
            "can_cancel_order_from_this_mcp": False,
        }
        fake_decision = {
            ...
            "blockers": ["This MCP has no broker execution capability; autonomous real-money execution cannot be enabled here."],
            "warnings": ["Paper/live sample size is still thin."],
            "minimum_before_cash_autonomy": ["Separate broker tool confirms account, buying power, positions, and open orders."],
            "operator_read": "Autonomous scanning and paper learning are enabled.",
            "review_only": True,
            "can_place_order_from_this_mcp": False,
            "can_cancel_order_from_this_mcp": False,
            "broker_action": False,
        ...
        client = TestClient(create_app())
        with patch("app.fallback_endpoints._get_strategy_module_registry", return_value=fake_registry), patch(
            "app.fallback_endpoints._get_shared_intelligence_layer", return_value=fake_intelligence
        ), patch("app.fallback_endpoints._get_autonomous_launch_decision", return_value=fake_decision):
            registry = client.get("/ops/strategy-modules?format=html")
            intelligence = client.get("/ops/shared-intelligence?tickers=SOFI&format=html")
            decision = client.get("/ops/autonomous-launch-decision?account_value=100&intended_cash=100&format=html")
            decision_json = client.get("/ops/autonomous-launch-decision?account_value=100&intended_cash=100")

        self.assertEqual(registry.status_code, 200)
        self.assertIn("Strategy Module Registry", registry.text)
        self.assertEqual(intelligence.status_code, 200)
        self.assertIn("Shared Intelligence Layer", intelligence.text)
        self.assertEqual(decision.status_code, 200)
        self.assertIn("Autonomous Launch Decision", decision.text)
        self.assertIn("DELAY_LAUNCH", decision.text)
        self.assertFalse(decision_json.json()["can_place_order_from_this_mcp"])
        self.assertFalse(decision_json.json()["result"]["capability_decision"]["autonomous_real_money_execution_enabled"])
    ...
    def test_real_cash_proof_gate_endpoint_is_review_only(self) -> None:
        fake_gate = {
            ...
            "decisions": {
                ...
                "manual_real_cash_review": "REAL_CASH_BLOCKED",
                "fully_autonomous_real_cash_execution": "AUTONOMOUS_EXECUTION_BLOCKED",
            },
            "proof_summary": {"proven_count": 2, "missing_count": 1, "blocked_count": 1, "warning_count": 0},
            "proof_items": [
                {"category": "build", "name": "Expected build is running", "status": "PROVEN", "evidence": "ok", "required_for": ["scan"]},
                {"category": "broker", "name": "Open orders checked", "status": "MISSING", "evidence": "required", "required_for": ["manual_cash_review"]},
                {"category": "broker_execution", "name": "Separate broker executor proven", "status": "BLOCKED", "evidence": "no executor", "required_for": ["autonomous_execution"]},
            ],
            "manual_cash_blockers": [{"name": "Open orders checked", "status": "MISSING"}],
            "autonomous_execution_blockers": [{"name": "Separate broker executor proven", "status": "BLOCKED"}],
            "warnings": [],
            "next_step": "Keep scanning and paper learning on.",
            "review_only": True,
            "can_place_order_from_this_mcp": False,
            "can_cancel_order_from_this_mcp": False,
            "broker_action": False,
        ...
        client = TestClient(create_app())
        with patch("app.fallback_endpoints._get_real_cash_proof_gate", return_value=fake_gate):
            html = client.get("/ops/real-cash-proof-gate?account_value=100&format=html")
            json_response = client.get("/ops/real-cash-proof-gate?account_value=100")

        self.assertEqual(html.status_code, 200)
        self.assertIn("Real-Cash Proof Gate", html.text)
        self.assertIn("AUTONOMOUS_EXECUTION_BLOCKED", html.text)
        self.assertEqual(json_response.status_code, 200)
    ...
    def test_journal_vault_status_and_restore_are_review_only(self) -> None:
        client = TestClient(create_app())
        checkpoint = {
            "status": "JOURNAL_CHECKPOINT_READY",
            "build_version": "2026.06.12-full-crypto-universe",
            "events": [
                {
                    ...
                    "event_type": "paper_exploration_run",
                    "payload": {"status": "PAPER_EXPLORATION_TRIALS_OPENED", "ticker": "HOOD"},
                }
            ],
        }

        status_json = client.get("/journal/vault")
        status_html = client.get("/journal/vault", headers={"accept": "text/html"})
        restore_json = client.post("/journal/vault", json={"checkpoint_text": json.dumps(checkpoint)})

        self.assertEqual(status_json.status_code, 200)
        self.assertEqual(status_json.json()["result"]["status"], "JOURNAL_VAULT_READY")
        self.assertFalse(status_json.json()["can_place_order_from_this_mcp"])
        self.assertEqual(status_html.status_code, 200)
        self.assertIn("Journal Vault", status_html.text)
        self.assertIn("Export checkpoint JSON", status_html.text)
        self.assertEqual(restore_json.status_code, 200)
        self.assertIn(restore_json.json()["result"]["status"], {"CHECKPOINT_RESTORE_READY", "CHECKPOINT_RESTORE_NO_NEW_EVENTS"})
        self.assertEqual(
            restore_json.json()["result"]["restored_count"] + restore_json.json()["result"]["skipped_duplicate_count"],
            1,
        )
